...
function menuAdmin(){return `<section class="panel"><div class="actions between"><h2>${t('products')}</h2><div class="actions"><button id="exportMenu">${t('exportFile')}</button><button id="importMenu">${t('importFile')}</button><button class="primary" id="addProduct">${t('add')}</button></div></div><input id="menuImportFile" class="hidden" type="file" accept=".csv,.json,text/csv,application/json">${app.data.products.map((p,i)=>`<div class="record-row"><div class="thumb-row"><img src="${esc(p.image||'./logo.png')}"><div><b>${esc(p.en)}</b><small>${esc(p.kh)} · ${money(p.price)} · ${esc(p.category)}</small></div></div><div class="actions"><button data-edit-product="${i}">${t('edit')}</button><button data-delete-product="${i}">${t('delete')}</button></div></div>`).join('')}</section>`}
function stockView(){return stockAdmin()}

function stockAdmin(){return `<section class="panel"><div class="actions between"><h2>${t('stock')}</h2><div class="actions"><button id="exportStock">${t('exportFile')}</button><button id="importStock">${t('importFile')}</button><button class="primary" id="addStock">${t('add')}</button></div></div><input id="stockImportFile" class="hidden" type="file" accept=".csv,.json,text/csv,application/json">${app.data.stock.map((p,i)=>`<div class="record-row"><div class="thumb-row"><img src="${esc(p.image||'./logo.png')}"><div><b>${esc(p.name)}</b><small>${p.qty} ${esc(p.unit||'')} · min ${p.min||0}</small></div></div><div class="actions"><button data-edit-stock="${i}">${t('edit')}</button><button data-delete-stock="${i}">${t('delete')}</button></div></div>`).join('')||`<div class="empty">${t('noData')}</div>`}</section>`}
...
document.addEventListener('change',async e=>{if(e.target?.id==='menuImportFile'){try{await importFileInto('menu',e.target.files?.[0])}catch(err){console.error(err);toast(err?.message||String(err))}e.target.value=''} if(e.target?.id==='stockImportFile'){try{await importFileInto('stock',e.target.files?.[0])}catch(err){console.error(err);toast(err?.message||String(err))}e.target.value='' }});
