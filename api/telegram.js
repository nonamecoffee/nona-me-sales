...
function buildMultipart(fields, fileBuffer) {
...
async function checkBot(botToken) {
  ...
  if (!me.response.ok || !me.data?.ok) {
    return {
      ...
      error: telegramError(me.data, me.raw || 'Telegram bot token is invalid or unavailable.')
    };
  }
...
export default async function handler(req, res) {
  ...
  if (route === 'health' || (!route && req.method === 'GET')) {
    ...
    if (!botToken) return json(res, 500, {
      ok: false,
    ...
    try {
      ...
      if (!checked.ok) return json(res, 502, {
        ok: false,
      ...
      return json(res, 200, {
        ...
        bot: checked.bot.username || null,
        botId: checked.bot.id || null,
  ...
  if (route === 'test') {
    ...
    try {
      ...
      const tx = await telegramCall(botToken, 'sendMessage', {
        chat_id: chatId,
      ...
      if (!tx.response.ok || !tx.data?.ok) {
        return json(res, 502, {
          ...
          error: telegramError(tx.data, tx.raw || 'Telegram could not send the test message.'),
          bot: checked.bot.username || null
        });
      }
  ...
  if (route === 'send' || route === 'report') {
    ...
    try {
      ...
      if (imageBuffer.length > 9_500_000) return json(res, 413, {
        ok: false,
      ...
      const multipart = buildMultipart({
        chat_id: chatId,
        caption: cleanText(body.caption).slice(0, 900),
        disable_notification: 'false'
      }, imageBuffer);

      const photo = await telegramCall(botToken, 'sendPhoto', multipart, true);
      if (!photo.response.ok || !photo.data?.ok) return json(res, 502, {
        ok: false,
        code: 'SEND_PHOTO_FAILED',
        error: telegramError(photo.data, photo.raw || 'Telegram could not send the report image.'),
        bot: checked.bot.username || null
      });

      const messageIds = [];
      for (const part of chunkText(body.text, 3900)) {
        const tx = await telegramCall(botToken, 'sendMessage', { chat_id: chatId, text: part });
        if (!tx.response.ok || !tx.data?.ok) return json(res, 502, {
          ok: false,
          code: 'SEND_TEXT_FAILED',
          error: telegramError(tx.data, tx.raw || 'Image was sent, but the report text could not be sent.'),
          image_sent: true,
          bot: checked.bot.username || null
        });
  ...
  return json(res, 404, {
    ok: false,
    code: 'UNKNOWN_ROUTE',
    error: 'Unknown Telegram API route. Use /api/telegram/health, /api/telegram/test, or /api/telegram/send.'
  });
};
